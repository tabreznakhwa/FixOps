...
interface Customer { id: string; full_name: string; company_name: string | null; mobile_number: string }
...
export function EditAMCForm({ contractId, customers, defaults }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [services, setServices] = useState<string[]>(defaults.services_included ?? [])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      renewal_reminder_date: fd.get('renewal_reminder_date') || null,
      notes: fd.get('notes') as string,
    }
    if (!body.customer_id) { setError('Please select a customer'); setLoading(false); return }
    try {
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        ...
        <div>
          ...
          <select name="customer_id" required defaultValue={defaults.customer_id} className={inputClass}>
        ...
        <div>
          ...
          <select name="contract_type" defaultValue={defaults.contract_type ?? 'comprehensive'} className={inputClass}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" name="start_date" required defaultValue={defaults.start_date} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>End Date <span className="text-red-500">*</span></label>
            <input type="date" name="end_date" required defaultValue={defaults.end_date} className={inputClass} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="contract_amount" min="0" step="0.01"
              defaultValue={defaults.contract_amount} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Billing Frequency</label>
            <select name="billing_frequency" defaultValue={defaults.billing_frequency} className={inputClass}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select name="payment_terms" defaultValue={String(defaults.payment_terms)} className={inputClass}>
          ...
          <div>
            <label className={labelClass}>Renewal Reminder Date</label>
            <input type="date" name="renewal_reminder_date"
              defaultValue={defaults.renewal_reminder_date ?? ''} className={inputClass} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="visits_included" min="0"
              defaultValue={defaults.visits_included} className={inputClass} />
          </div>
          <div className="flex items-end pb-2">
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" name="parts_included"
                defaultChecked={defaults.parts_included}
                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
