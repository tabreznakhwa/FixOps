...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      payment_mode?: string
      notes?: string
      items: { inventory_item_id: string; description: string; quantity: number; unit_cost: number }[]
    }
    ...
    if (items.some(it => !it.inventory_item_id)) return NextResponse.json({ error: 'All items must be linked to inventory' }, { status: 400 })
