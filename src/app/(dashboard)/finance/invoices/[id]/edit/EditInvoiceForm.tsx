...
interface WorkOrder { id: string; work_order_number: string; final_amount: number; customer_id: string }
interface InventoryItem {
  id: string; item_code: string; item_name: string; selling_price: number
  unit_of_measure: string; current_stock: number; category: string | null
}
...
interface Props {
  invoice: {
    ...
    due_date: string | null; ref_number: string | null; print_name: string | null; discount_amount: number; notes: string | null
...
const PRESET_SERVICES = [
  { group: '❄️ AC Services', items: [
    { label: 'Filter Cleaning', price: 5 }, { label: 'Gas Top-up (R22)', price: 15 },
    { label: 'Gas Top-up (R32 / R410A)', price: 20 }, { label: 'Drain Block Removal', price: 10 },
    { label: 'AC Full Service (Clean + Check)', price: 25 }, { label: 'Split AC Installation', price: 40 },
    { label: 'Cassette / Ceiling AC Installation', price: 60 }, { label: 'AC Uninstallation', price: 20 },
    { label: 'Thermostat Replacement', price: 15 }, { label: 'Capacitor Replacement', price: 12 },
    { label: 'Fan Motor Replacement', price: 35 }, { label: 'Compressor Diagnostic', price: 10 },
    { label: 'PCB / Control Board Repair', price: 30 }, { label: 'Remote Control Replacement', price: 8 },
    { label: 'Gas Leak Detection', price: 15 }, { label: 'Duct Cleaning (per unit)', price: 20 },
    { label: 'Annual AC Maintenance Contract', price: 60 },
  ]},
  { group: '🔧 Plumbing', items: [
    { label: 'Leak Detection & Repair', price: 10 }, { label: 'Pipe Replacement', price: 20 },
    { label: 'Tap / Faucet Replacement', price: 15 }, { label: 'Toilet Repair / Flush Fix', price: 15 },
    { label: 'Water Heater Installation', price: 25 }, { label: 'Drain Unblocking / Cleaning', price: 10 },
    { label: 'Kitchen Sink Repair', price: 12 }, { label: 'Shower Mixer Replacement', price: 20 },
    { label: 'Water Tank Cleaning', price: 30 },
  ]},
  { group: '⚡ Electrical', items: [
    { label: 'Wiring Repair', price: 15 }, { label: 'Switch / Socket Replacement', price: 8 },
    { label: 'MCB / Breaker Replacement', price: 12 }, { label: 'Light Fixture Installation', price: 8 },
    { label: 'Ceiling Fan Installation', price: 15 }, { label: 'DB Panel Inspection', price: 10 },
    { label: 'Earth Leakage / RCD Testing', price: 12 }, { label: 'Cable Pulling & Trunking', price: 20 },
  ]},
  { group: '🔨 General', items: [
    { label: 'Inspection / Site Visit', price: 5 }, { label: 'Labour Charge (per hour)', price: 10 },
    { label: 'Transportation / Call-out Fee', price: 3 }, { label: 'Emergency Call-out Surcharge', price: 15 },
    { label: 'Annual Maintenance Contract (AMC)', price: 50 }, { label: 'Warranty Repair', price: 0 },
  ]},
]
...
export function EditInvoiceForm({ invoice, items: initialItems, customers, workOrders, inventoryItems }: Props) {
  ...
  const [printName, setPrintName] = useState(invoice.print_name ?? '')
  ...
  const switchItemType = useCallback((index: number, type: LineType) => {
    setItems(prev => {
      const next = [...prev]
      next[index] = { ...next[index], type, inventory_item_id: '', service_label: '', description: '', unit_price: '' }
      return next
    })
  }, [])
  ...
  const selectInventoryItem = useCallback((index: number, itemId: string) => {
    ...
    setItems(prev => {
      const next = [...prev]
      next[index] = { ...next[index], inventory_item_id: itemId,
        description: inv ? inv.item_name : '',
        unit_price: inv && inv.selling_price > 0 ? String(inv.selling_price) : '' }
      return next
    })
  }, [inventoryItems])
  ...
  const selectService = useCallback((index: number, label: string) => {
    ...
    setItems(prev => {
      const next = [...prev]
      next[index] = { ...next[index], service_label: label,
        description: label,
        unit_price: svc && svc.price > 0 ? String(svc.price) : '' }
      return next
    })
  }, [])

  const addItem = () => setItems(prev => [...prev, { type: 'custom', inventory_item_id: '', service_label: '', description: '', quantity: '1', unit_price: '' }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (items.some(it => !it.description.trim() || !it.unit_price)) {
      setError('All line items must have a description and unit price'); return
    }
    setLoading(true)
    try {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select className={inputClass} value={customerId} onChange={e => { setCustomerId(e.target.value); setWorkOrderId('') }} required>
          ...
          {filteredWorkOrders.length > 0 && (
            <div className="md:col-span-2">
              ...
              <select className={inputClass} value={workOrderId} onChange={e => {
                const wid = e.target.value; setWorkOrderId(wid)
                if (wid) { const wo = workOrders.find(w => w.id === wid); if (wo) setCustomerId(wo.customer_id) }
              }}>
                <option value="">No work order</option>
                {filteredWorkOrders.map(wo => (
                  <option key={wo.id} value={wo.id}>{wo.work_order_number} — KWD {wo.final_amount.toFixed(3)}</option>
                ))}
              </select>
            </div>
          )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          {items.map((item, index) => {
            ...
            return (
              <div key={index} className="border border-slate-200 rounded-xl p-3 space-y-2.5 bg-slate-50">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex rounded-lg border border-slate-200 bg-white overflow-hidden text-xs font-semibold divide-x divide-slate-200">
                    {(['custom', 'inventory', 'service'] as const).map(t => (
                      <button key={t} type="button" onClick={() => switchItemType(index, t)}
                        className={`px-3 py-1.5 transition-colors ${item.type === t ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
                ...
                {item.type === 'inventory' && (
                  <select value={item.inventory_item_id} onChange={e => selectInventoryItem(index, e.target.value)} className={inputClass}>
                    ...
                    {Object.entries(invByCategory).sort(([a], [b]) => a.localeCompare(b)).map(([cat, catItems]) => (
                      <optgroup key={cat} label={cat}>
                        {catItems.map(inv => (
                          <option key={inv.id} value={inv.id}>{inv.item_name} — KWD {inv.selling_price.toFixed(3)} (Stock: {inv.current_stock})</option>
                        ))}
                      </optgroup>
                    ))}
                  </select>
                ...
                {item.type === 'service' && (
                  <select value={item.service_label} onChange={e => selectService(index, e.target.value)} className={inputClass}>
                    <option value="">Select service…</option>
                    {PRESET_SERVICES.map(group => (
                      <optgroup key={group.group} label={group.group}>
                        {group.items.map(svc => (
                          <option key={svc.label} value={svc.label}>{svc.label}{svc.price > 0 ? ` — KWD ${svc.price.toFixed(3)}` : ' — Free / Custom price'}</option>
                        ))}
                      </optgroup>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-12 md:col-span-7">
                    <input type="text" placeholder={item.type === 'custom' ? 'Description *' : 'Description (auto-filled, editable)'}
                      className={inputClass} value={item.description}
                      onChange={e => updateItem(index, 'description', e.target.value)} required />
                  </div>
                  <div className="col-span-4 md:col-span-2">
                    <input type="number" placeholder="Qty" min="0.01" step="0.01"
                      className={`${inputClass} text-right`} value={item.quantity}
                      onChange={e => updateItem(index, 'quantity', e.target.value)} required />
                  </div>
                  <div className="col-span-5 md:col-span-2">
                    <input type="number" placeholder="0.000" min="0" step="0.001"
                      className={`${inputClass} text-right`} value={item.unit_price}
                      onChange={e => updateItem(index, 'unit_price', e.target.value)} required />
                  </div>
        ...
        <button type="button" onClick={addItem} className="mt-4 flex items-center gap-2 text-sm text-blue-600 font-semibold hover:text-blue-700 transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
          ...
          <div>
            ...
            <input type="number" min="0" step="0.001" className={inputClass} value={discountAmount} onChange={e => setDiscountAmount(e.target.value)} />
          </div>
        </div>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
          ...
          <div className="space-y-2.5 text-sm">
            <div className="flex justify-between text-slate-600">
              <span>Subtotal</span><span className="font-medium text-slate-900">{fmt(subtotal)}</span>
            </div>
            {discount > 0 && (
              <div className="flex justify-between text-slate-600">
                <span>Discount</span><span className="font-medium text-green-700">− {fmt(discount)}</span>
              </div>
            )}
            <div className="border-t border-slate-200 pt-2.5 flex justify-between">
              <span className="font-bold text-slate-900 text-base">Total</span>
              <span className="font-bold text-blue-600 text-base">{fmt(total)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <textarea className={`${inputClass} resize-none`} rows={4} placeholder="Internal notes…" value={notes} onChange={e => setNotes(e.target.value)} />
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <label className={labelClass}>Terms & Conditions</label>
          <textarea className={`${inputClass} resize-none`} rows={4} placeholder="Payment terms, warranty, etc." value={termsAndConditions} onChange={e => setTermsAndConditions(e.target.value)} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pb-6">
        ...
        <a href={`/finance/invoices/${invoice.id}`}
