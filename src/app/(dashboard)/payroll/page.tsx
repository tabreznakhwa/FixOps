...
export default async function PayrollPage() {
  ...
  const { data: staffRaw } = await (supabase as any)
    .from('staff')
    .select('id, full_name, designation, department, basic_salary, housing_allowance, transport_allowance, food_allowance, other_allowance, overtime_eligible, overtime_rate, employment_status')
  ...
  const totals = staff.reduce(
    (acc, s) => ({
      ...
      housing: acc.housing + (s.housing_allowance ?? 0),
      transport: acc.transport + (s.transport_allowance ?? 0),
      food: acc.food + (s.food_allowance ?? 0),
      other: acc.other + (s.other_allowance ?? 0),
    }),
    { basic: 0, housing: 0, transport: 0, food: 0, other: 0 },
  )

  const totalMonthlyBill = totals.basic + totals.housing + totals.transport + totals.food + totals.other
              ...
              <table className="w-full min-w-[900px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    ...
                    <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3">Housing</th>
                    <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3">Transport</th>
                    <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3">Food</th>
                    <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3">Other</th>
                    <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3">OT Rate</th>
                  ...
                  {staff.map((s) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-right text-sm text-slate-600">{formatCurrency(s.housing_allowance ?? 0)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-slate-600">{formatCurrency(s.transport_allowance ?? 0)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-slate-600">{formatCurrency(s.food_allowance ?? 0)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-slate-600">{formatCurrency(s.other_allowance ?? 0)}</td>
                        <td className="px-4 py-3.5 text-right text-sm text-slate-500">
                          {s.overtime_eligible && s.overtime_rate
                            ? `${formatCurrency(s.overtime_rate)}/hr`
                            : '—'}
                        </td>
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t border-slate-200">
                    ...
                    <td className="px-4 py-3 text-right text-sm font-semibold text-slate-700">{formatCurrency(totals.housing)}</td>
                    <td className="px-4 py-3 text-right text-sm font-semibold text-slate-700">{formatCurrency(totals.transport)}</td>
                    <td className="px-4 py-3 text-right text-sm font-semibold text-slate-700">{formatCurrency(totals.food)}</td>
                    <td className="px-4 py-3 text-right text-sm font-semibold text-slate-700">{formatCurrency(totals.other)}</td>
