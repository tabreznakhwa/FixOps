...
export function SupplierSelector({ suppliers, selectedId }: Props) {
  ...
  function navigate(supplierId: string) {
    ...
    if (supplierId) params.set('supplier_id', supplierId); else params.delete('supplier_id')
    router.push(`/suppliers/vendor-ledger?${params.toString()}`)
    setOpen(false)
    setQuery('')
  }
