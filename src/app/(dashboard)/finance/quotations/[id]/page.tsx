      ...
      <div className="hidden print:block p-10 text-slate-900">
        <div className="flex items-start justify-between pb-6 border-b-2 border-slate-800 mb-8">
          <div className="flex items-start gap-4">
            {org?.logo_url ? <img src={org.logo_url} alt={org.name} className="h-16 w-auto object-contain" /> : (
              <div className="w-14 h-14 bg-blue-600 rounded-xl flex items-center justify-center text-white text-xl font-bold flex-shrink-0">{org?.name?.slice(0, 2).toUpperCase() ?? 'FO'}</div>
            )}
            <div>
          ...
          <div className="text-right">
            ...
            <p className="text-xl font-mono font-bold text-slate-900 mt-1">{quotation.quotation_number}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Quote To</p>
          ...
          <div className="text-right">
            <div className="inline-block text-left space-y-1.5">
              {[
                ...
                <div key={label} className="flex gap-8 text-sm"><span className="text-slate-500 w-28">{label}</span><span className="font-semibold text-slate-900">{value}</span></div>
              ))}
            </div>
          </div>
        </div>
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr key={item.id} className="border-b border-slate-100">
                ...
                <td className="px-4 py-2.5 text-sm text-right font-semibold text-slate-900">{formatCurrency(item.total_price)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end mb-8">
          <div className="w-72 space-y-2 text-sm">
            ...
            <div className="border-t-2 border-slate-800 pt-2 flex justify-between text-base font-bold"><span>Total (KWD)</span><span>{formatCurrency(quotation.total_amount)}</span></div>
          </div>
        </div>

        {(quotation.notes || quotation.terms_and_conditions) && (
          <div className="border-t border-slate-200 pt-6 space-y-3 text-sm">
            {quotation.notes && <div><p className="font-bold text-slate-700 mb-1">Notes</p><p className="text-slate-600 whitespace-pre-wrap">{quotation.notes}</p></div>}
            {quotation.terms_and_conditions && <div><p className="font-bold text-slate-700 mb-1">Terms & Conditions</p><p className="text-slate-600 whitespace-pre-wrap">{quotation.terms_and_conditions}</p></div>}
          </div>
        )}
        ...
        <div className="mt-12 grid grid-cols-2 gap-12 border-t border-slate-200 pt-6">
          ...
          <div><div className="border-b border-slate-400 mb-2 h-10" /><p className="text-xs text-slate-500">Customer Approval Signature & Stamp</p></div>
        </div>
        <p className="text-center text-xs text-slate-400 mt-8">This quotation is valid until the date shown above unless otherwise agreed.</p>
      </div>
